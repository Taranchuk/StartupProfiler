...
namespace StartupProfiler
{
	...
	public class ReportWindow : Window
	{
		...
		private void CopyModListToClipboard()
		{
			...
			{
				sb.AppendLine($"{modImpactData.TotalImpactTime.ToStringDecimalIfSmall()}s ({(modImpactData.TotalImpactTime / totalImpact).ToStringPercent()}) - {modImpactData.mod.Name + " - " + modImpactData.mod.PackageId.ToLower()}");
			}
